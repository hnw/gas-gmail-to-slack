...
まず Node.js 20.19 以上 (corepack 有効) と Visual Studio Code をインストールしましょう。
...
`pnpm clasp:push` は内部で `pnpm build` を実行し、`dist/code.js` と `dist/appsscript.json` を `clasp push` します。

初回デプロイ後は、以下の開発コマンドが使えます。
